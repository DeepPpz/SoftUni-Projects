...
def product_quantity_ordered():
    total_ordered = (Product.objects
                              .annotate(total_quantity=Sum('orderproduct__quantity')).exclude(total_quantity=0)
                              .order_by('-total_quantity'))
    output = []
    
    for product in total_ordered:
        output.append(f'Quantity ordered of {product.name}: {product.total_quantity}')
...
def ordered_products_per_customer():
    ...
    for order in all_orders:
        ...
        for order_product in order.orderproduct_set.all():
            output.append(f'- Product: {order_product.product.name}, Category: {order_product.product.category.name}')
    
    return '\n'.join(output)


def filter_products():
    all_products = Product.objects.filter(Q(is_available=True) & Q(price__gt=3.00)).order_by('-price', 'name')
    
    output = []
    for product in all_products:
        output.append(f'{product.name}: {product.price}lv.')

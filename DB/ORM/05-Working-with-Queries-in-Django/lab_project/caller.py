...
def find_authors_nationalities():
    ...
    for author in authors_with_nationalities:
        end_text.append(f'{author.first_name} {author.last_name} is {author.nationality}')
...
def order_books_by_year():
    ...
    for book in all_books:
        end_text.append(f'{book.publication_year} year: {book.title} by {book.author}')
...
def delete_review_by_id(review_id: int):
    ...
    return f'Review by {name} was deleted'


def filter_authors_by_nationalities(searched_nationality: str):
    searched_authors = Author.objects.filter(nationality=searched_nationality).order_by('first_name', 'last_name')
    end_text = []
    
    for author in searched_authors:
        end_text.append(f'{author.first_name} {author.last_name}' if author.biography is None else author.biography)
    
    return '\n'.join(end_text)


def filter_authors_by_birth_year(first_year: int, secong_year: int):
    searched_authors = Author.objects.filter(birth_date__year__range=(first_year, secong_year)).order_by('-birth_date')
    end_text = []
    
    for author in searched_authors:
        end_text.append(f'{author.birth_date}: {author.first_name} {author.last_name}')
    
    return '\n'.join(end_text)


def change_reviewer_name(old_name: str, new_name: str):
    reviewers = Review.objects.filter(reviewer_name=old_name)
